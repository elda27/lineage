...
export class SqliteTagRepository implements TagRepository {
  ...
  async all(workspaceId: string) {
    const rows = await this.db.select<Row[]>(
      `SELECT t.*,v.view_id,a.recipe_name,a.ownership
       FROM tag_definitions t
       LEFT JOIN view_bindings v ON v.tag_id=t.id
       LEFT JOIN automation_bindings a ON a.tag_id=t.id
       WHERE (t.workspace_id=? OR t.workspace_id='local') AND t.deleted_at IS NULL`,
      [workspaceId],
    );
    return rows.map(map);
  }
  ...
  async update(id: string, value: TagUpdate) {
    await invoke<void>("tag_update", {
      id,
      value,
